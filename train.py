...
def train():
    ...
    #UNet.load_state_dict(torch.load('./check_newmodel/50000.pth'))
    STN = SpatialTransformer((160,192,160)).to(device)
    UNet.train()
    STN.train()
    # 模型参数个数
    ...
    train_files = glob.glob(os.path.join('/storage/caoxiaoling/LPBA/train_2/*/'))
    ...
    for i in range(1, args.n_iter + 1):
        ...
        input_moving,input_surf,input_fixed,input_surf_2 = iter(DL).next()
        # [B, C, D, W, H]
        input_moving = input_moving.to(device).float()
        input_fixed = input_fixed.to(device).float()
        input_surf = input_surf.to(device).float()
        input_surf_2 = input_surf_2.to(device).float()

        # Run the data through the model to produce warp and flow field
        flow_m2f = UNet(input_moving, input_fixed)
        m2f = STN(input_moving, flow_m2f)

        # Calculate loss+0.75*nj_loss
        sim_loss = sim_loss_fn(m2f, input_fixed)
        grad_loss = grad_loss_fn(flow_m2f)
        nj_loss=nj_loss_fn(flow_m2f)
