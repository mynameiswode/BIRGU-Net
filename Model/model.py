...
class UpsampleBlock(nn.Module):
    def __init__(self, channel_in, channel):
        ...
        self.conv1 = nn.Conv3d(channel_in, channel, kernel_size=3, stride=1, padding=1, bias=True)
        self.bn1 = nn.BatchNorm3d(channel)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv3d(channel, channel, kernel_size=3, stride=1, padding=1, bias=True)
        self.bn2 = nn.BatchNorm3d(channel)
        self.conv3 = nn.Conv3d(channel_in, channel, kernel_size=3, stride=1, padding=1, bias=True)
...
class U_Network(nn.Module):
    def __init__(self, dim, enc_nf, dec_nf, bn=None, full_size=True):
        ...
        self.layer1=nn.Sequential(BasicBlock(16),DownsampleBlock(16, 32))
        self.layer2=nn.Sequential(BasicBlock(32),DownsampleBlock(32, 64))
        self.layer3=nn.Sequential(BasicBlock(64),DownsampleBlock(64, 128))

        # Encoder functions4,16, 32, 32, 32
        self.enc = nn.ModuleList()

        self.enc.append(self.conv_block(dim, 2, 16, 4, 2, batchnorm=True))
        self.enc.append(self.layer1)
        self.enc.append(self.layer2)
        self.enc.append(self.layer3)



        self.dec1=nn.Sequential(UpsampleBlock(192, 64))
        self.dec2=nn.Sequential(UpsampleBlock(96, 32))
        self.dec3=nn.Sequential(UpsampleBlock(48, 16))# Decoder functions
        self.dec = nn.ModuleList()
        self.dec.append(self.dec1)  # 1
        self.dec.append(self.dec2)  # 2
        self.dec.append(self.dec3)  # 3
